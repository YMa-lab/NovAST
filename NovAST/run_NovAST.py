...
def run_NovAST(config_file="default_config.yaml", **override_kwargs):
    ...
    if has_spatial(adata_test) == False:
    ...
    dataset, labeled_y, unlabeled_y, _ = load_paired_dataset(adata_train, adata_test, args)
    ...
    for i in range(1, args.rounds+1):
        ...
        if 'exploration' in args.training_mode:
            adata_all, adata_unlabeled, predicted_labels, labeled_y, z_test, _ = identify_novel_cells_exploration(labeled_y, z_train, unlabeled_y, z_test, args, filedic)

        elif 'evaluation' in args.training_mode:
            adata_all, adata_unlabeled, predicted_labels, labeled_y, z_test, unlabeled_y_name, _ = identify_novel_cells_evaluation(labeled_y, z_train, unlabeled_y, z_test, args, filedic)

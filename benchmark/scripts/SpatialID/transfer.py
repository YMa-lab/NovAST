...
from .trainer import Base, SpatialTrainer, DnnTrainer
...
class Transfer(Base):
    ...
    def annotation(self,
        ...
        print('  After Preprocessing All self.st_data : %d cells × %d genes.' % (self.st_data.shape[0], self.st_data.shape[1]))
        print('  After Preprocessing All gene_mat : %d cells × %d genes.' % (gene_mat.shape[0], gene_mat.shape[1]))

        # only for stereo-seq or other sequencing-based ST which has large # of genes
        if self.st_data.shape[1] > 10000:
        ...
        print('  Location shapee : %d cells × %d genes.' % (cell_coo.shape[0], cell_coo.shape[1]))
        ...
        print('  pseudo label shape : %d cells × %d genes.' % (data.y.shape[0], data.y.shape[1]))
        print('  data shape : %d cells × %d genes.' % (data.x.shape[0], data.x.shape[1]))
        ...
        print('  after training st shape : %d cells × %d genes.' % (self.st_data.shape[0], self.st_data.shape[1]))
        # Inference.
        predictions, feat = trainer.infer(data)
        ...
        self.st_data.obsm['feat'] = feat

        del self.st_data.uns
        self.st_data.obs = self.st_data.obs.astype(str)
    ...
    def annotation_st2st(self,
        ...
        # print(" as numpy:", predictions.cpu().numpy(), predictions.dtype, predictions.shape)

        celltype_pred = pd.Categorical([self.st_data.uns['pseudo_classes'][i] for i in predictions])

        # Save results.
        result = pd.DataFrame({'cell': self.st_data.obs_names.tolist(), 'celltype_pred': celltype_pred})
        map_func = np.vectorize(lambda x: mapping_dict.get(x, x))
        celltype_pred_mapped = map_func(celltype_pred)
        result['celltype_pred_mapped'] = celltype_pred_mapped
        result.to_csv(osp.join(self.output_path, "model.csv"), index=False)
        self.st_data.obs['celltype_pred'] = pd.Categorical(celltype_pred)
        self.st_data.obs['celltype_pred_mapped'] = pd.Categorical(celltype_pred_mapped)
        self.st_data.obsm['feat'] = feat
        ...
        if show_results:
            ...
            sc.pl.spatial(self.st_data, img_key=None, color=['celltype_pred'], spot_size=spot_size, show=False)
            plt.savefig(osp.join(self.output_path, "celltype_pred.pdf"), bbox_inches='tight', dpi=150)
            sc.pl.spatial(self.st_data, img_key=None, color=['celltype_pred_mapped'], spot_size=spot_size, show=False)
            plt.savefig(osp.join(self.output_path, "celltype_pred_mapped.pdf"), bbox_inches='tight', dpi=150)
        print("\nSpatialID successfully finished the power source annotation!")

...
class BOL:
    ...
    def train_epoch_ours(self, args, model, device, dataset, optimizer, epoch):
        ...
        unlabeled_data = ClusterData(unlabeled_graph, num_parts=args.num_parts_test, recursive=False)
        ...
        has_novel = args.total_classes > args.shared_classes
        ...
        for batch_idx, unlabeled_x in enumerate(unlabeled_loader):
            ...
            # --- CE loss on labeled data (always active, probe 1) ---
            ...
            if has_novel:
                # --- Probe-2 losses: only when novel class slots exist ---
                ...
                # --- Closed-set: only supervised CE on probe 1 ---
    ...
    def pred(self, epoch):
        self.model.eval()
        has_novel = self.args.total_classes > self.args.shared_classes
        ...
        with torch.no_grad():
            ...
            output_s, output_t, out_feat = self.model(unlabeled_graph_cp)
            ...
            if has_novel:
                ...
                # Closed-set: predictions from probe 1 only
                conf_s, pred_s = output_s.max(1)
                preds = np.append(preds, pred_s.cpu().numpy())
                preds_open = np.append(preds_open, pred_s.cpu().numpy())
        ...
        unseen_acc = cluster_acc(preds[unseen_mask], targets[unseen_mask]) if unseen_mask.any() else 0.0

        overall_acc2 = cluster_acc(preds_open, targets)
        seen_acc2 = cluster_acc(preds_open[seen_mask], targets[seen_mask])
        unseen_acc2 = cluster_acc(preds_open[unseen_mask], targets[unseen_mask]) if unseen_mask.any() else 0.0
        ...
        print('In the old {}-th epoch, Test overall acc2 {:.4f}, seen acc2 {:.4f}, unseen acc2 {:.4f}'.format(epoch,
                                                                                                              ...
                                                                                                              seen_acc2,
                                                                                                              unseen_acc2))
        return preds_open, out_feat.cpu().numpy()
    ...
    def train(self):
        ...
        if has_novel:
            ...
            # Closed-set: no SSKM needed, set dummy attributes so data objects are complete
            n_unlabeled = self.dataset.unlabeled_data.x.shape[0]
            self.dataset.unlabeled_data.test_label = torch.zeros(n_unlabeled, dtype=self.dataset.unlabeled_data.y.dtype)
            self.dataset.unlabeled_data.test_mask = torch.zeros(n_unlabeled, dtype=self.dataset.unlabeled_data.y.dtype)

        # Set the optimizer
        optimizer = optim.Adam(self.model.parameters(), lr=self.args.lr, weight_decay=self.args.wd)

        # Number of training epochs. Configurable via the SCBOL_EPOCHS env var so the
        # value used is recorded per-job in the sbatch script (`export SCBOL_EPOCHS=...`).
        # Default 1000 preserves the original behaviour for any earlier-completed seeds.
        # Loss converges by ~epoch 100 (CE/Cluster/Align/Regular are flat from 100->1000
        # on merfish_development_coarse), so remaining runs use SCBOL_EPOCHS=100 for ~10x
        # speedup with negligible change in the trained model.
        n_epochs = int(os.environ.get("SCBOL_EPOCHS", "1000"))
        print(f"[scBOL] training for {n_epochs} epochs (SCBOL_EPOCHS env)", flush=True)

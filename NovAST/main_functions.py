...
def load_paired_dataset(adata_train, adata_test, args):
    ...
    (labeled_X, labeled_y, inverse_dict_train, unlabeled_X, unlabeled_y, unlabeled_y_c, inverse_dict_test) = outputs
    ...
    return dataset, labeled_y, unlabeled_y, unlabeled_X
...
def identify_novel_cells_evaluation(labeled_y, z_train, unlabeled_y, z_test, args, filepath):
    start_time = time.time()
...
def identify_novel_cells_exploration(labeled_y, z_train, unlabeled_y, z_test, args, filepath):
    start_time = time.time()
    print("Step2 started!")
    # get combined latent space, transform it to an anndata, and add feautres (i.e., labled or not)
    z_all = np.concatenate((z_train, z_test), axis=0)
...
def vote(args):
    ...
    for i in range(1, args.rounds+1):
        ...
        predictions = adata.obs['final_prediction']
        label_spreading =  adata.obs['label_prop']
        all_predictions.append(predictions)
        all_labelspread.append(label_spreading)

    pattern = re.compile(r"novel\d+")
    def contains_novel(column):
        return any(pattern.search(x) for x in column)
    
    all_predictions_setting_df = pd.DataFrame(all_predictions)
    all_labelspread_setting_df = pd.DataFrame(all_labelspread)
    column_indices =all_predictions_setting_df.apply(contains_novel)
    novel_counts = all_predictions_setting_df.loc[:, column_indices].apply(lambda col: sum(1 for x in col if pattern.search(x)))
    for col in all_predictions_setting_df.columns[column_indices]:
        if novel_counts[col] < args.vote_thresh:
            all_predictions_setting_df[col] = all_labelspread_setting_df[col]
    for i in range(1, args.rounds+1):
        print(i)
        row_array = all_predictions_setting_df.iloc[i-1].to_numpy()
        output_path = os.path.join(args.savedir, f"seed{i}")
        adata = sc.read_h5ad(os.path.join(output_path, "adata_unlabeled_end_of_step2.h5ad"))

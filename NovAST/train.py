...
class NovAST_train:
    ...
    def train_epoch(self, args, model, device, dataset, optimizer, epoch):
        ...
        print(f"loss: {sum_loss / n_batches:.6f}")
        print(f"Reconstruction loss: {sum_recons / n_batches:.6f}")
        print(f"Similarity loss: {sum_sim / n_batches:.6f}")
        print(f"MMF loss: {sum_mmf / n_batches:.6f}")
        return loss_dict
